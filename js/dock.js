(function () {
  var ITEMS = [
    ...
    {
      label: 'LinkedIn',
      href: 'https://www.linkedin.com/in/braidena',
      target: '_blank',
    ...
    {
      label: 'Email',
      href: 'mailto:koaxx316@gmail.com',
      target: '',
  ...
  dock.innerHTML = ITEMS.map(function (item) {
    return '<a href="' + item.href + '"' +
      ...
      ' class="dock-item" aria-label="' + item.label + '" style="position:relative">' +
      '<span class="dock-label">' + item.label + '</span>' +
      '<div class="dock-icon">' + item.svg + '</div>' +
      '</a>';
  }).join('');

  document.body.appendChild(dock);

  // Magnification on mousemove
  dock.addEventListener('mousemove', function (e) {
    ...
    items.forEach(function (item) {
      ...
      var maxDist = 90;
      var scale = dist < maxDist ? 1 + (1 - dist / maxDist) * 0.65 : 1;
